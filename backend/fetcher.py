...
Fetches articles from RSS feeds listed in feeds.yaml, summarizes them using an LLM
(OpenAI ChatGPT) to create a blog topic prompt, and then passes that prompt to the
agent system to generate a blog post.
"""
...
class BlogTopicGenerator:
    ...
    async def summarize_to_topic(self, articles: List[ArticleData]) -> str:
        """Use OpenAI to summarize articles and formulate a blog topic prompt."""
        if not articles:
        ...
        prompt = f"""
Dựa trên bộ sưu tập các bài báo tin tức gần đây sau đây, vui lòng:

1. Tóm tắt các chủ đề và chủ đề chính được đề cập trong các bài viết này.
2. Xác định các mẫu hoặc xu hướng kết nối giữa các câu chuyện.
3. Đề xuất một chủ đề bài viết blog hấp dẫn kết hợp các chủ đề này.
4. Cung cấp một lời nhắc chi tiết cho người viết blog để tạo một bài viết hấp dẫn về chủ đề này.

Các bài viết:
{articles_summary}

Vui lòng phản hồi với:
- Tóm tắt ngắn gọn các bài viết
- Chủ đề bài viết blog được đề xuất
- Lời nhắc viết chi tiết phù hợp cho trình tạo blog AI
"""

        self.logger.info("Sending articles summary to OpenAI for topic generation...")

        try:
            result = await self.llm_client.generate(
                prompt,
                system_prompt="Bạn là một chiến lược gia chủ đề blog sáng tạo, người xác định các kết nối trong tin tức và đề xuất các lời nhắc viết hấp dẫn.",
                temperature=0.7
            )
            return result if result else "News Summary and Blog Topic"
        except Exception as e:
            self.logger.error(f"Error generating topic with OpenAI: {e}")
            return "News Summary and Blog Topic"
